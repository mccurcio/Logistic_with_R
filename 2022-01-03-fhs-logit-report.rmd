...
1. This report investigates the risk factors leading to cardiovascular disease (CVD) using the Framingham Heart Study data. The study included 4,133 participants with 13 factors total over 10 years.[^1] 
...
1. Using the Framingham Heart Study data, we find seven (7) factors and their related odds leading to cardiovascular disease.
...
1. Logistic regression does not simply predict whether a person has a disease or not. Logistic regression can help determine which variable(s) are associated with cardiovascular disease. 

1. Logistic regression produces probabilities of being in one case or another. Based on our regression model, Using these probabilities, one case (P) versus (1-P), we can generalize to find the odds, $\small P/(1-P)$ for each variable.
...
- This longitudinal study includes 4,133 participants over a ten year period. 

- The model is fit with 13 independent variables. 

- Patients were evaluated for the absence or presence of Cardiac Heart Disease After Ten Years**, abbreviated `TenYearCHD` between 1948-1960.[^2]
...
- By testing seven (7) significant risk factors we see that this model has a p-value = 0.0. Therefore this model does 
```{r}
...
- We can conclude the seven (7) parameters are significant and useful in describing cardiovascular disease.
...
- By holding all other values constant we create a dataframe that investigates the odds given Prevalence of Stroke
...
1.	The logistic regression model accuracy score is 0.8501. So, the model does a very good job in predicting whether or not an individual will acquire cardiovascular disease.
